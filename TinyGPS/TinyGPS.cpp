...
bool TinyGPS::term_complete()
{
  if (_is_checksum_term)
  {
    char checksum = 16 * from_hex(_term[0]) + from_hex(_term[1]);
    if (checksum == _parity)
    {
      if (_gps_data_good)
      {
...
void TinyGPS::get_datetime(unsigned long *date, unsigned long *time)
{
  if (date) *date = _date;
  if (time) *time = _time;
}
...
void TinyGPS::crack_datetime(int *year, char *month, char *day, 
  char *hour, char *minute, char *second, char *hundredths)
{
